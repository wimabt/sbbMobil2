...
class ArView(
                ...
                object : ModelNode(
                    ...
                    override fun onMoveBegin(detector: MoveGestureDetector, e: MotionEvent): Boolean {
                        if (handlePans) {
                            ...
                            // FIX (vendored): upstream `defaultResult`'u return
                            // etmiyordu → hep false dönüp sürüklemeyi engelliyordu.
                            return defaultResult
                        }
                        return false
                    }
    ...
    try {
        // FIX (vendored): Guard `if (handlePans || handleRotation)` KALDIRILDI.
        // Native pan/rotation kapalıyken bile Flutter'dan gelen transform
        // (ölçek/taşıma/döndürme) uygulanmalı; aksi halde pinch-scale ve
        // programatik yerleştirme/rotasyon hiç etki etmiyordu (model session
        // origin'de ham GLB pozunda kalıyordu → "ters" + pinch çalışmıyor).
        run {
            ...
            nodesMap[name]?.let { node ->
                newTransformation?.let { transform ->
                    ...
                    // FIX (vendored): Eski kod rotasyonu matristen EULER açısına
                    // çözüp veriyordu (kayıplı + eksen/birim belirsizliği) → model
                    // "baş aşağı/ters" geliyordu. Artık ölçeği kolon normlarından
                    // ayıklayıp saf 3x3 rotasyonu TAM QUATERNION'a (Shepperd) çevirip
                    // node.transform'a doğrudan veriyoruz — kayıpsız, eksen-doğru.
                    val sx = kotlin.math.sqrt((transform[0]*transform[0] + transform[1]*transform[1] + transform[2]*transform[2]).toFloat())
                    val sy = kotlin.math.sqrt((transform[4]*transform[4] + transform[5]*transform[5] + transform[6]*transform[6]).toFloat())
                    val sz = kotlin.math.sqrt((transform[8]*transform[8] + transform[9]*transform[9] + transform[10]*transform[10]).toFloat())
                    val q = quaternionFromColumnMajor(transform, sx, sy, sz)
    ...
    private fun handleAddAnchor(
        ...
        try {
            ...
            if (anchorType == 0) { // Plane Anchor
                ...
                if (name != null && transform != null && transform.size == 16) {
                    try {
                        // FIX (vendored): Eski kod rotasyonu bozuk `deserializeMatrix4`
                        // (Euler, w=1f) ile alıyordu → anchor yönü yanlıştı. Artık
                        // konumu + TAM quaternion'u (Shepperd) çıkarıp anchor pozunu
                        // doğru kuruyoruz. Anchor konum+dönüşü taşır (ARCore takip
                        // eder); ölçek node tarafında (scaleToUnits) kalır.
                        val sx = kotlin.math.sqrt((transform[0]*transform[0] + transform[1]*transform[1] + transform[2]*transform[2]).toFloat())
                        val sy = kotlin.math.sqrt((transform[4]*transform[4] + transform[5]*transform[5] + transform[6]*transform[6]).toFloat())
                        val sz = kotlin.math.sqrt((transform[8]*transform[8] + transform[9]*transform[9] + transform[10]*transform[10]).toFloat())
                        ...
                        if (anchor != null) {
                            ...
                            try {
                                anchorNode.transform =
                                    Transform(position = position, quaternion = q)
                            } catch (e: Exception) {
...
/// Kolon-major 4x4 transform'dan (ölçek baked-in) saf rotasyonu çıkarıp
/// tam quaternion'a çevirir (Shepperd metodu — 4 trace durumu da ele alınır).
/// [sx]/[sy]/[sz] kolon normlarıdır (ölçek); 3x3 rotasyon bunlara bölünerek
/// normalize edilir. Euler'e göre kayıpsız ve eksen-doğrudur.
private fun quaternionFromColumnMajor(
    m: ArrayList<Double>,
    sx: Float,
    sy: Float,
    sz: Float,
): Quaternion {
    val ex = if (sx == 0f) 1f else sx
    val ey = if (sy == 0f) 1f else sy
    val ez = if (sz == 0f) 1f else sz
    // r[row][col] = m[col*4 + row] / scale_col
    val r00 = (m[0].toFloat()) / ex;  val r10 = (m[1].toFloat()) / ex;  val r20 = (m[2].toFloat()) / ex
    val r01 = (m[4].toFloat()) / ey;  val r11 = (m[5].toFloat()) / ey;  val r21 = (m[6].toFloat()) / ey
    val r02 = (m[8].toFloat()) / ez;  val r12 = (m[9].toFloat()) / ez;  val r22 = (m[10].toFloat()) / ez
    ...
    return when {
        ...
        r00 > r11 && r00 > r22 -> {
            val s = kotlin.math.sqrt(1f + r00 - r11 - r22) * 2f
            Quaternion(0.25f * s, (r01 + r10) / s, (r02 + r20) / s, (r21 - r12) / s)
        }
        r11 > r22 -> {
            val s = kotlin.math.sqrt(1f + r11 - r00 - r22) * 2f
            Quaternion((r01 + r10) / s, 0.25f * s, (r12 + r21) / s, (r02 - r20) / s)
        }
        else -> {
            val s = kotlin.math.sqrt(1f + r22 - r00 - r11) * 2f
