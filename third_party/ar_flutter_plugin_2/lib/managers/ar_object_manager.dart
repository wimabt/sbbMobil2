...
class ARObjectManager {
  ...
  onInitialize() {
    _channel.invokeMethod<void>('init', {}).catchError((Object e) {
      // Eskiden bu hata hiç yakalanmıyordu → "MissingPluginException(... init
      // on channel arobjects_N)" konsola düşüp kayboluyordu. Artık etiketli.
      // ignore: avoid_print
      print('🛰️ [AR-DIAG] objectManager.init ${e.runtimeType} → $e');
    });
  }
  ...
  Future<bool?> addNode(ARNode node, {ARPlaneAnchor? planeAnchor}) async {
    try {
      ...
      // ignore: avoid_print
      print('🛰️ [AR-DIAG] addNode PlatformException → '
          'code=${e.code} message=${e.message} details=${e.details}');
      return false;
    } catch (e) {
      // MissingPluginException vb. PlatformException olmayan hatalar buraya düşer
      // (native köprü hiç yoksa). Eskiden sessizce yutuluyordu.
      // ignore: avoid_print
      print('🛰️ [AR-DIAG] addNode ${e.runtimeType} → $e');
      rethrow;
    }
  }
