...
class ARSessionManager {
  ...
  ///
  /// NOT: Android native tarafı (ArView.kt handleGetCameraPose) pozu
  /// `{position:{x,y,z}, rotation:{x,y,z,w}}` MAP'i olarak döndürüyor — 16'lık
  /// kolon-major List DEĞİL. Eski kod `invokeMethod<List>` + `MatrixConverter`
  /// kullandığı için her çağrıda `_Map is not a subtype of List` cast hatası
  /// atıp null dönüyordu (→ AR yerleştirme yönü bozuk, taşıma çalışmıyor, log
  /// spam'i). Artık her iki formatı da çözüyoruz.
  Future<Matrix4?> getCameraPose() async {
    try {
      final dynamic raw = await _channel.invokeMethod('getCameraPose', {});
      if (raw == null) return null;
      // Alternatif/iOS formatı: 16 elemanlı kolon-major liste.
      if (raw is List) {
        return MatrixConverter().fromJson(raw);
      }
      // Android formatı: {position:{x,y,z}, rotation(quaternion):{x,y,z,w}}.
      if (raw is Map) {
        final pos = Map<dynamic, dynamic>.from(raw['position'] as Map);
        final rot = Map<dynamic, dynamic>.from(raw['rotation'] as Map);
        double d(Object? v) => (v as num).toDouble();
        final translation = Vector3(d(pos['x']), d(pos['y']), d(pos['z']));
        final quaternion =
            Quaternion(d(rot['x']), d(rot['y']), d(rot['z']), d(rot['w']));
        return Matrix4.compose(translation, quaternion, Vector3(1, 1, 1));
      }
      return null;
    } catch (e) {
      print('Error caught (getCameraPose): ' + e.toString());
      return null;
    }
  }
